...
public class TripEventHandler implements PersonStuckEventHandler, LinkLeaveEventHandler, VehicleEntersTrafficEventHandler,
		TeleportationArrivalEventHandler, TransitDriverStartsEventHandler, ActivityEndEventHandler, ActivityStartEventHandler,
		PersonDepartureEventHandler, PersonArrivalEventHandler {

	private Set<Id<Person>> drivers = new HashSet<>();
	...
	public Map<Id<Person>, List<Trip>> getTripToPerson() {
		return tripToPerson;
	}
	...
	public void handleEvent(ActivityEndEvent event) {
		if (isInteraction(event.getActType()) || drivers.contains(event.getPersonId())) return;

		if (isBeforeDvrpSchedule(event.getActType())) {
			drivers.add(event.getPersonId());
			return;
		}
	...
	public void handleEvent(PersonArrivalEvent event) {
		...
		} else if (!drivers.contains(event.getPersonId())) {
	...
	public void handleEvent(PersonDepartureEvent event) {
		...
		if (drivers.contains(event.getPersonId())) return;
	...
	private boolean isBeforeDvrpSchedule(String type) {
		return type.equals("BeforeVrpSchedule");
	}
